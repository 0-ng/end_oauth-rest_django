...
import { addmyUser, querymyUser, removemyUser, updatemyUser, querymyUserVerboseName, querymyUserListDisplay, querymyUserDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updatemyUser(value, current_id);
      hide();
  ...
  useEffect(() => {
    querymyUserDisplayOrder().then(r => {
  ...
  useEffect(() => {
    querymyUserListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, querymyUser, table_columns, 'my user-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索my user" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'my user-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => querymyUser({ ...params, sorter, filter })}
